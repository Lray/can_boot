...
def write_info(args: argparse.Namespace,
    ...
    (args.out_dir / "package.info").write_text(
        "\n".join([
            "source=mcu-boot-valid-target-runtime",
            "boot_valid=yes",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a signed MCU MCUboot image and OTA or provisioning bundle."
    )
    ...
    parser.add_argument("--provisioning", action="store_true",
                        help="emit a confirmed-trailer image.bin for Boot YMODEM initial "
                             "provisioning instead of a MCU update package input")
