...
- MCU has been flashed with and is running the UDS server under review.
- Without ISO-TP padding:
  ...
  - interpretation for review: this is MCU raw echo fallback, not a UDS positive response.
- With ISO-TP padding:
...
- The trace above is historical evidence, not a timing-profile acceptance value. Under
  ...
  The current MCU/gateway profile requires `01 F4` for `P2*=5000 ms`; a new board
...
- Linux SocketCAN/can-utils default short-DLC ISO-TP single-frame requests are legal.
  The MCU should accept both legal short-DLC single frames and padded single frames.
...
- Wait for MCU-side fix/retest of short-DLC ISO-TP receive and raw echo fallback behavior.
